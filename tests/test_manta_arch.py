...
def test_final_channels_autoexpand(oc):
    floor = MANTA_PRESETS["medium"]["final_channels"]  # 16
    m = manta_from_preset("medium", n_bins=256, bins_pad=64, output_channels=oc, tower_height=1)
...
def test_high_channel_builds_and_roundtrips(tmp_path):
    ...
    m = manta_from_preset("medium", n_bins=256, bins_pad=64, output_channels=13, tower_height=1)
    assert _final_channels(m) == 32
    x = torch.randn(1, 1032, 2 ** (1 + 1) * (256 + 2 * 64))
    with torch.no_grad():
        assert m(x).shape == (1, 13, 256, 256)
    p = tmp_path / "m.pth"
    save_manta_checkpoint(m, str(p), model_params=MANTA_PRESETS["medium"])
    ck = torch.load(p, map_location="cpu", weights_only=False)
    m2 = manta_from_preset("medium", n_bins=256, bins_pad=64,
